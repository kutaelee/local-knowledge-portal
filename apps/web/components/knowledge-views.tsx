...
const textByLocale = {
  ko: {
    ...
    knowledge: {
      ...
      feed: "개발자 피드", feedSubtitle: "검증된 새 정보를 기술 설명·문제 해결 글로 공유하며 매일 18시에 핵심 배움을 고릅니다.",
  ...
  en: {
    ...
    knowledge: {
      ...
      feed: "Developer feed", feedSubtitle: "Verified updates become explainers and troubleshooting posts, with one key lesson selected at 18:00.",
      feedEmpty: "No verified technical insight is ready to share yet.",
      dailySummary: "Today's technical note", embeddedEvidence: "Embedded evidence",
...
type DeveloperFeedPost = {
  id: string;
  project: string;
  post_type: "activity" | "daily_summary";
  ...
  source_manifest: {
    ...
    passing_check_count?: number;
    documents?: Array<{ document_id: string; filename: string; embedded_at: string }>;
  };
  source_embedding_to: string | null;
...
export function KnowledgeCases({ locale }: { locale: Locale }) {
  ...
  const [tab, setTab] = useState<"cases" | "candidates" | "journal" | "feed">("journal");
  ...
  const feed = useQuery({
    queryKey: ["developer-feed", locale, page, selectedProject],
    queryFn: () => api<{ items: DeveloperFeedPost[]; total: number }>(
  ...
  return <section>
    ...
    <div className="tabs">
      ...
      <button className={tab === "feed" ? "active" : ""} onClick={() => {
        ...
        setSelectedCategory(""); setSelectedTag(""); setPage(1);
      }}>{text.feed}<span className="tab-count">{feed.data?.total ?? 0}</span></button>
    </div>
    {tab === "feed" && <div className="developer-feed">
      <div className="developer-feed-intro">
        <Activity size={18} /><div><strong>{text.feed}</strong><span>{text.feedSubtitle}</span></div>
      </div>
      {Array.from(
        (feed.data?.items ?? []).reduce((groups, post) => {
          const key = post.thread_root_id || post.id;
          const current = groups.get(key) ?? [];
          current.push(post);
          groups.set(key, current);
          return groups;
        }, new Map<string, DeveloperFeedPost[]>()),
