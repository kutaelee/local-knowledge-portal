...
평가를 통과하지 못한 E4B는 자동 게시 권한을 얻지 못한다. 이 경우 운영자는 같은
모델 계열의 `gemma4:12b`를 우선 후보로 사용하고, 필요하면 `qwen3:14b`를 대안으로
설정해 같은 평가를 다시 수행한다. 상위 모델도 digest·prompt·생성 파라미터가 다른
별도 qualification revision이며, 모델 다운로드와 전환은 실패 결과와 용량 확인 후
명시적으로 수행한다.
...
`test_pass`, `build_pass`, `command_success`, `command_failure`는 Bash·PowerShell·
shell command처럼 실제 명령을 실행한 도구에서만 만들 수 있다. `apply_patch` 본문에
`tests/` 경로나 명령 예제가 있다는 이유로 실행 근거를 만들지 않는다. 과거에 잘못
파생된 실행 근거는 원본 활동을 삭제하지 않고 `verified=false`와 무효화 사유를
기록하며, 해당 근거 없이는 gate를 통과하지 못하는 자동 후보를 `activity_only`로
격리한다.
